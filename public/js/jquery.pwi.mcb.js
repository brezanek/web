/**
 * Picasa Webalbum Integration jQuery plugin - MC Brezanek customization
 * @author Pavel Sýkora
 * @revision 2.0.0
 * @date 2012-08-22
 * @license Creative Commons Attribution-Share Alike 3.0 Czech Republic License - http://creativecommons.org/licenses/by-sa/3.0/cz/
 *
...
(function ($) {
    ...
    $.fn.pwi = function (opts) {
            ...
            $.each($albumsToShow, function(i, n) {
                ...
                if (settings.showAlbumTitles) {
                    ...
                    $scAlbumTitle.append(((n.title.$t.length > settings.showAlbumTitlesLength) ?
                            ...
                            (settings.showAlbumPhotoCount ? "&nbsp;&nbsp;&nbsp;&nbsp;obrázků: " +
        ...
        function album(j) {
            ...
            if (settings.mode != 'album' && settings.mode != 'keyword') {
                tmp = $("<p class='pwi_album_backlink'>" + settings.labels.albums + "</p>").bind('click.pwi', function (e) {
                    e.stopPropagation();
                    $("#mcbgallery-title").html(settings.galleryTitle);
                    getAlbums();
                    return false;
                });
                $scPhotos.append(tmp);
            ...
            if (settings.showAlbumDescription) {
                $("#mcbgallery-title").html($at);
                $scPhotosDesc = $("<div class='pwi_album_description'/>");
            //     $scPhotosDesc.append("<h3 class='title'>" + $at + "</h3>");
                $scPhotosDesc.append("<div class='details'> obrázků: " + $np +
        ...
        function getAlbums() {
            if (settings.albumstore.feed) {
                ...
                var $u = strings.picasaUrl + settings.username +
                    '?kind=album&access=' + settings.albumTypes + 
        ...
        function getAlbum() {
            if (settings.photostore[settings.album]) {
                ...
                $('#mcbgallery-title').ScrollTo({onlyIfOutside: true}); // this is an MCB-specific hack !! (depends on the ScrollTo plugin) 
            }
            return $self;
        }
    ...
    $.fn.pwi.defaults = {
        ...
        username: 'galerie@mcbrezanek.cz', //-- Must be explicitly set!!!
        ...
        labels: {
            photo: "photo",
            photos: "photos",
            downloadphotos: "Stáhnout fotografii",
            albums: "Zpět na seznam alb",
            noalbums: "Bohužel zde nejsou žádná alba",
            page: "Stránka",
            prev: "Předchozí",
            next: "Následující",
            showPermaLink: "Ukázat trvalý odkaz",
            showMap: "Ukázat mapu",
        ...
        blockUIConfig: {
            message: "<div class='lbLoading pwi_loader'>načítá se...</div>",
            css: "pwi_loader"
